...
export type DownloadReleaseData = {
  ...
  >;
  /** Android ships a single arm64-v8a APK. */
  android: { apk?: DownloadAsset };
};
...
function parseLatestRelease(payload: LatestReleaseResponse): DownloadReleaseData {
  ...
  for (const asset of payload.assets ?? []) {
    ...
    // Android ships one APK, so it is matched before the arch lookup rather
    // than being keyed by architecture like the desktop builds.
    if (asset.name.includes('-Android-') && asset.name.endsWith('.apk')) {
      data.android.apk = parsedAsset;
      continue;
    }
